...
pub fn infer_task_type(task_id: &str) -> TaskType {
    ...
    // Check intent/chain first — they may contain other keywords in their description
...
pub fn infer_outcome(response_text: &str, tool_rounds: u32) -> Outcome {
    ...
    // Very short responses with no tool usage suggest a failure or empty result
...
pub fn infer_domain(task_type: &TaskType, task_id: &str) -> String {
    match task_type {
        ...
        TaskType::Intent => {
            // Try to infer from the intent description in the ID
            let id = task_id.to_lowercase();
